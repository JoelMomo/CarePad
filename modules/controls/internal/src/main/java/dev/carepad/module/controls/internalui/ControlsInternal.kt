...
class ControlsInternalController(
        ...
        when (guidedStage) {
            ...
            GuidedStage.DIGITAL -> {
                ...
                digitalOutcomes[target.button] = when {
                    ...
                    canInterpretDigitalTarget(activeSession, target.button) -> Outcome.NOT_DETECTED
    ...
    private fun canInterpretDigitalTarget(activeSession: ControlsSession, button: ControlButton): Boolean =
        button in activeSession.device.keys || (button in dpadButtons && activeSession.mapping.hat != null)
...
@Composable
private fun GuidedCountdown(seconds: Int) {
    Surface(
